# 👋 Rémy Mozelle

**Développeur back-end PHP / Symfony | Laravel | Drupal | Full remote depuis La Réunion**

## 👨‍💻 À propos

Développeur back-end PHP / Symfony basé à La Réunion, avec **8 ans d’expérience**, dont **5 années en full remote** au sein d’équipes basées en métropole chez [Ecedi](https://www.linkedin.com/company/ecedi/) du groupe [Ctrl-a](https://www.linkedin.com/company/ctrl%C2%B7a/)** et [Simplon Prod](https://www.simplon.co/).

📍 Localisation : Réunion, France - Full remote (France métropolitaine, Océan Indien DOM-TOM)

## 💻 Stack Technique

### Backend & Langages
- PHP (Symfony, Laravel)
- Node.js

### Frontend
- JavaScript
- React
- HTML
- CSS

### CMS
- Drupal
- Craft CMS

### Tests & Qualité
- PHPUnit
- Codeception

### Bases de données
- MySQL

### Environnements
- Linux
- Docker (via DDEV)
- Notions de conteneurisation

### Versioning & Collaboration
- Git
- GitHub
- GitLab

## 💼 Compétences métier

### 🌐 Développement Web
- Conception, développement et évolution de sites et d'applications web
- Développement d'API REST
- Intégration de services tiers
- Adaptation des solutions aux besoins métiers

### ✅ Contrôle qualité
- Documentation technique
- Réalisation de tests unitaires et fonctionnels
- Application des bonnes pratiques de développement
- Maintenance évolutive

### 🖥️ Exploitation & Environnements
- Administration d'environnements Linux
- Gestion des environnements de développement
- Notions autour des pratiques DevOps

### 🤝 Travail en équipe
- Collaboration en équipe pour la résolution de problématiques techniques
- Participation aux réunions et échanges visant l’amélioration des processus et méthodologies de développement

## 🌍 Domaines d'expérience

- Service public
- Associatif
- Handicap ([Certifié Opquast, Avancé](https://directory.opquast.com/fr/certificat/1YSWMP/))

## 📚 En savoir plus
🔗 [LinkedIn]([https://www.linkedin.com/...](https://www.linkedin.com/in/remy-mozelle/))
🔗 [ITW sur mon parcours en 2020](https://www.linkedin.com/pulse/r%25C3%25A9my-le-ph%25C3%25A9nix-de-simplonprod-tom-lann/)
<!--
**RemyMozelle/RemyMozelle** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
